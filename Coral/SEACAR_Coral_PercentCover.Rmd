...
for(i in 1:length(file_list)){
  if(i==1){
    data <- fread(file_list[i], sep=",", header=TRUE, stringsAsFactors=FALSE,
                  na.strings="")
    data$`[PercentCover-SpeciesComposition_%]`<- as.numeric(data$`[PercentCover-SpeciesComposition_%]`)
    
  } else{
    temp <- fread(file_list[i], sep=",", header=TRUE,
                            stringsAsFactors=FALSE, na.strings="")
    temp$`[PercentCover-SpeciesComposition_%]`<- as.numeric(temp$`[PercentCover-SpeciesComposition_%]`)
...
1. Set parameter names to `Percent Cover`
2. `[PercentCover-SpeciesComposition_%]` column is renamed `perccov`
3. Sets units
4. Replaces any NA values that were imported as blank character strings or `"NA"` character strings with the proper NA value
5. Removes any data that is not coral
  + Only looks for `Octocoral` or `Unspecifiedcoral`
  
6. Removes rows that contains NA values in `ManagedAreaName`, `GenusName`, `SpeciesName`, `Month`, `Year`, `SpeciesGroup1`, `perccov`, and `SampleDate`
...
11. Turns `SampleDate` into a date structure easily interpretted by R
12. Gets list of managed areas to be analyzed.
...
data <- data[SpeciesGroup2=="Octocoral"|SpeciesGroup2=="Unspecifiedcoral", ]
...
# Remove rows with missing perccov
data <- data[!is.na(data$perccov),]
...
# Remove timestamp and makes sure SampleDate is being stored as a Date object
data$SampleDate <- sub(" 12:00AM", "", data$SampleDate)
data$SampleDate <- as.Date(data$SampleDate, format="%b %d %Y")


# Creates a variable with the names of all the managed areas that contain
# species observations
MA_Include <- unique(data$ManagedAreaName[!is.na(data$perccov)])
...
MA_YM_Stats <- data %>%
   ...
   summarize(ParameterName=parameter,
             N_Data=length(na.omit(perccov)),
             Min=min(perccov),
             Max=max(perccov),
             Median=median(perccov),
             Mean=mean(perccov),
             StandardDeviation=sd(perccov),
...
for(i in 1:n){
  ...
  # Perform LME for relation between perccov and Year for current managed area
  AnyCoral<-lme(perccov ~ Year,
...
if(n==0){
   ...
   for (i in 1:n) {
      ...
      y_range <- max(plot_data$perccov) - min(plot_data$perccov)
      ...
      # maximum value.
      y_max <- max(plot_data$perccov)+(0.1*y_range)
      
      
      # Creates plot object using plot_data.
      # Data is plotted as symbols with connected lines.
      p1 <- ggplot(data=plot_data) +
        geom_jitter(aes(x=Year, y=perccov), shape=21, size=2,
