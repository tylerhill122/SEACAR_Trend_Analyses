...
c_names <- c("AreaID", "ManagedAreaName", "Median", "Independent", 
...
if(n==0){
   ...
   for (i in 1:n) {
      ...
      p <- KT.Stats$p[KT.Stats$ManagedAreaName==MA_Include[i]]
      ...
      p1 <- ggplot(data=plot_data,
                   aes(x=DecDate, y=ResultValue, fill=VQ_Plot)) +
         geom_point(shape=21, size=3, color="#333333", alpha=0.75) +
      ...
      print(ggarrange(p0, KTset, ncol=1, heights=c(0.1, 1)))
      cat('\n')
      print(KT.Stats[KT.Stats$ManagedAreaName==MA_Include[i], ] %>%
         select(N_Data, Median, tau, SennSlope, SennIntercept, p, Trend) %>%
         kable(format="latex") %>%
