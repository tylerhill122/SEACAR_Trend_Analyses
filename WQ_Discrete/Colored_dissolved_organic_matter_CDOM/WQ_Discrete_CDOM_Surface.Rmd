---
title: "SEACAR Discrete Water Quality Analysis: Surface Colored Dissolved Organic Matter (CDOM)"
...
The purpose of this script is to analyze the discrete surface colored dissolved organic matter (CDOM) data that is created from the SEACAR database, apply filtering criteria, create summary plots, and perform seasonal Kendall Tau analysis for each program location and summary statistics for values measured at the desired depth.
...
data <- merge.data.frame(data, MA_Years[,c("ManagedAreaName", "Enough_Time")],
                         by = "ManagedAreaName")

data$Use_In_Analysis <- ifelse(data$Include == TRUE &
                                  data$Enough_Time == TRUE,
                               TRUE, FALSE)
...
tauSeasonal <- function(data, independent, stats.median, stats.minYear,
   ...
   KT <-c(unique(data$AreaID),
          unique(data$ManagedAreaName),
          parameter,
          depth,
          activity,
          independent,
...
c_names <- c("AreaID", "ManagedAreaName", "ParameterName", "RelativeDepth",
             "ActivityType", "Independent", "Median", "N", "EarliestYear",
             "LatestYear", "tau", "p", "SennSlope", "SennIntercept", "Trend")
if(n==0){
   KT.Stats <- data.frame(matrix(ncol=15, nrow=0))
...
if(n==0){
   ...
   for (i in 1:n) {
      ...
      mn_RV <- mean(plot_data$ResultValue[data$ResultValue <
                                        quantile(data$ResultValue, 0.98)])
      sd_RV <- sd(plot_data$ResultValue[data$ResultValue <
                                      quantile(data$ResultValue, 0.98)])
      x_scale <- ifelse(year_upper - year_lower > 30, 10, 5)
      y_scale <- mn_RV + 4 * sd_RV
