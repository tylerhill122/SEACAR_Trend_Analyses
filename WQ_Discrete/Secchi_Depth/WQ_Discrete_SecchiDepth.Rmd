---
title: "SEACAR Discrete Water Quality Analysis: Secchi Depth"
...
The purpose of this script is to analyze the discrete secchi depth data that is created from the SEACAR database, apply filtering criteria, create summary plots, and perform seasonal Kendall Tau analysis for each program location and summary statistics for values measured at the desired depth.
...
After filtering, the amount of data impacted by the H (for dissolved oxygen & pH in program 476), I, Q, and U value qualifiers. A variable is also created that determines if scatter plot points should be a different color based on value qualifiers of interest. The value qualifier S indicates that the Secchi depth was visible on bottom.
...
MA_YM_Stats <- data[data$Use_In_Analysis == TRUE, ] %>%
   ...
   summarize(N = length(ResultValue),
             ...
             N_VoB = length(grep("S", ValueQualifier)),
...
MA_YM_Stats <- as.data.table(MA_YM_Stats[order(MA_YM_Stats$ManagedAreaName,
                                               MA_YM_Stats$Year,
                                               MA_YM_Stats$Month), ])
fwrite(MA_YM_Stats, paste0(out_dir,"/", param_name, "_", file_date,
...
MA_Y_Stats <- as.data.table(MA_Y_Stats[order(MA_Y_Stats$ManagedAreaName,
                                             MA_Y_Stats$Year), ])
fwrite(MA_Y_Stats, paste0(out_dir,"/", param_name, "_", file_date,
...
MA_M_Stats <- as.data.table(MA_M_Stats[order(MA_M_Stats$ManagedAreaName,
                                             MA_M_Stats$Month), ])
fwrite(MA_M_Stats, paste0(out_dir,"/", param_name, "_", file_date,
...
tauSeasonal <- function(data, independent, stats.median, stats.minYear,
   ...
   KT <-c(unique(data$ManagedAreaName),
          independent,
          stats.median,
          nrow(data),
          stats.minYear,
...
c_names <- c("ManagedAreaName", "Independent", "Median", "N", "EarliestYear",
             "LatestYear", "tau", "p", "SennSlope", "SennIntercept", "Trend")
if(n==0){
   KT.Stats <- data.frame(matrix(ncol=11, nrow=0))
   colnames(KT.Stats) <- c_names
   fwrite(KT.Stats, paste0(out_dir,"/", param_name, "_", file_date,
...
p1 <- ggplot(data = data[data$Include==TRUE,],
   ...
   {if(inc_H==TRUE){
      ...
      scale_color_manual(values = c("S"= "#F8766D", "U"= "#00BFC4",
                                    "SU" = "#7CAE00"), na.value="black")
   } else {
