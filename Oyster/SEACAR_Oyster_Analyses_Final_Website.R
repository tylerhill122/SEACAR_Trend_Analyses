...
diagnosticplots <- function(model, indicator, managedarea, sizeclass = "", historical = FALSE){
  ...
  #save chains plots
  jpeg(filename = here::here(paste0("output/Figures/", ind, "_AllDates_GLMM_", ma, "_PDistandMChains_", ifelse(sizeclass != "", paste0(size, "_"), ""), ifelse(historical == TRUE, "hist_", "_"), Sys.Date(), ".png")), 
  ...
  if(!is.na(postpc)){
    ...
    ggsave(here::here(paste0("output/Figures/", ind, "_AllDates_GLMM_", ma, "_PPcheck_", ifelse(sizeclass != "", paste0(size, "_"), ""), ifelse(historical == TRUE, "_hist_", "_"), Sys.Date(), ".png")),
           postpc,
           width = 6,
           height = 6,
           units = "in",
...
meplots <- function(models, data, indicator, managedarea, sizeclass = "", zoom = FALSE){
  ...
  if(ind == "SH"){
    ...
    ggsave(here::here(paste0("output/Figures/SH_AllDates_GLMM_", ma, "_MEPrand_", ifelse(sizeclass != "", paste0(size, "_"), ""), Sys.Date(), ".png")),
           plot1,
           width = 8,
           height = 4,
           units = "in",
    ...
    if(zoom == TRUE){
      ...
      ggsave(here::here(paste0("output/Figures/SH_AllDates_GLMM_", ma, "_MEPrand_", ifelse(sizeclass != "", paste0(size, "_"), ""), Sys.Date(), "_Zoom.png")),
             cplot,
             width = 8,
             height = 4,
             units = "in",
    ...
    if("Region.y" %in% if(present == "hist"){names(histplot_1)} else{names(liveplot_1)}){
      ...
      ggsave(here::here(paste0("output/Figures/SH_AllDates_GLMM_", ma, "_MEPrand_", ifelse(sizeclass != "", paste0(size, "_"), ""), "MeanRes_", Sys.Date(), ".png")),
             meanshplot,
             width = 8,
             height = 4,
             units = "in",
  ...
  if(ind == "Den"){ 
    ...
    ggsave(here::here(paste0("output/Figures/Den_AllDates_GLMM_", ma, "_", type,"_MEPrand_", ifelse(sizeclass != "", paste0(size, "_"), "raw_"), Sys.Date(), ".png")),
           plot1,
           width = 8,
           height = 4,
           units = "in",
    ...
    if("Region.y" %in% names(denplots)){
      ...
      ggsave(here::here(paste0("output/Figures/Den_AllDates_GLMM_", ma, "_MEPrand_", ifelse(sizeclass != "", paste0(size, "_"), "raw_"), "MeanRes_", Sys.Date(), ".png")),
             meandenplot,
             width = 8,
             height = 4,
             units = "in",
    ...
    if("RelYear:Region.y" %in% names(denplots)){
      ...
      ggsave(here::here(paste0("output/Figures/Den_AllDates_GLMM_", ma, "_MEPrand_", ifelse(sizeclass != "", paste0(size, "_"), "raw_"), "RelYrbyRegion_", Sys.Date(), ".png")),
             intplot,
             width = 10,
             height = 10,
             units = "in",
  ...
  if(ind == "Pct"){
    ...
    ggsave(here::here(paste0("output/Figures/Pct_AllDates_GLMM_", ma, "_MEPrand_raw_", Sys.Date(), ".png")),
           plot1,
           width = 8,
           height = 4,
           units = "in",
    ...
    if("Region.y" %in% names(pctplots)){
      ...
      ggsave(here::here(paste0("output/Figures/Pct_AllDates_GLMM_", ma, "_MEPrand_raw_MeanRes_", Sys.Date(), ".png")),
             meanpctplot,
             width = 8,
             height = 4,
             units = "in",
    ...
    if("RelYear:Region.y" %in% names(pctplots)){
      ...
      ggsave(here::here(paste0("output/Figures/Pct_AllDates_GLMM_", ma, "_MEPrand_raw_RelYrbyRegion_", Sys.Date(), ".png")),
             intplot,
             width = 10,
             height = 10,
             units = "in",
...
oysterraw[str_detect(MA_plotlab, "Pine Island Sound"), `:=` (MA_plotlab = ifelse(str_detect(ProgramLocationID, "Reference") | str_detect(ProgramLocationID, "Control"), "Pine Island Sound Aquatic Preserve_Natural", "Pine Island Sound Aquatic Preserve_Restored"),
...
oyster_sh_summ <- oysterraw %>%
  ...
  group_by(ManagedAreaName, nyrpar, LiveDate_Qualifier, SizeClass, HabitatClassification) %>%
  summarise(N_Data=length(ShellHeight_mm[!is.na(ShellHeight_mm)]),
            N_Years=length(unique(LiveDate[!is.na(LiveDate) & !is.na(ShellHeight_mm)])),
...
oyster_den_summ <- oysterraw %>%
  ...
  summarise(N_Data=length(Density_m2[!is.na(Density_m2)]),
            N_Years=length(unique(LiveDate[!is.na(LiveDate) & !is.na(Density_m2)])),
...
oyster_pct_summ <- oysterraw %>%
  ...
  summarise(N_Data=length(PercentLive_pct[!is.na(PercentLive_pct)]),
            N_Years=length(unique(LiveDate[!is.na(LiveDate) & !is.na(PercentLive_pct)])),
...
for(i in unique(oysterraw$ManagedAreaName)){
  oysterraw[ManagedAreaName == i & !is.na(LiveDate), `:=` (RelYear = (LiveDate - min(LiveDate)) + 1)]
}
# Oyster Shell Height -----------------------------------------------------
...
# Create model results tables and save diagnostic plots and marginal effects plots
data1 <- smm_sh35to75
models1 <- list(smm_sh35to75_glmm_hist) 
#modresults(data, models, "Size class", meplotzoom = TRUE)
...
# Create model results tables and save diagnostic plots and marginal effects plots
data2 <- smm_sho75
models2 <- list(smm_sho75_glmm_hist) 
#modresults(data, models, "Size class", meplotzoom = TRUE)
modresultssh(data1, models1, data2, models2, "Size class", meplotzoom = FALSE)
...
PI_R <- nrow(subset(oysterresults, oysterresults$managed_area=="Pine Island Sound Aquatic Preserve" &
                      oysterresults$indicator=="Density" & oysterresults$habitat_class=="Restored"))
oysterresults$group[is.na(oysterresults$group)] <- NA
if(PI_R>0){
  oysterresults$group[oysterresults$managed_area=="Pine Island Sound Aquatic Preserve" &
                        oysterresults$indicator=="Density" &
                        oysterresults$habitat_class=="Restored"] <- c(NA, NA, NA)
  
  oysterresults$term[oysterresults$managed_area=="Pine Island Sound Aquatic Preserve" &
                       oysterresults$indicator=="Density" &
                       oysterresults$habitat_class=="Restored"] <- c("(Intercept)", "RelYear", "QuadSize_m2")
}
...
fwrite(oysterresults, paste0("output/Tables/GLMM_AllDates_ModelResults_", Sys.Date(), ".csv"), sep=",")
saveRDS(oysterresults, here::here(paste0("output/Tables/GLMM_AllDates_ModelResults_", Sys.Date(), ".rds")))
