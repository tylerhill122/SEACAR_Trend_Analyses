...
meplots <- function(models, data, indicator, managedarea, sizeclass="",
  ...
  if(ind=="SH"){
    ...
    plot1 <- ggplot() +
      geom_point(data=data[!is.na(RelYear) & !is.na(LiveDate), ],
                  aes(x=RelYear, y=ShellHeight_mm), position=plot_jitter, 
    ...
    if(zoom==TRUE){
      ...
      ggsave(paste0("output/Shell_Height/Figures/Oyster_SH_GLMM_", ma,
                    "_Zoom.png"),
             cplot,
             width=8,
             height=4,
  ...
  if(ind=="Den"){ 
    nyrs <- max(data$RelYear)-min(data$RelYear)
...
meplotssh <- function(models1, data1, sizeclass1="", models2, data2,
  ...
  minyr_hist <- min(data1[!is.na(RelYear) & LiveDate_Qualifier=="Estimate",
                    ...
                    data2[!is.na(RelYear) & LiveDate_Qualifier=="Estimate",
                          RelYear])
  nyrs_hist <- (maxyr_hist)-(minyr_hist)
  if(minyr_hist > 0){
 ...
 breaks_hist <- seq(minyr_hist, maxyr_hist, brk)
  ...
  minyr_live <- min(data1[!is.na(RelYear) & LiveDate_Qualifier=="Exact",
                    ...
                    data2[!is.na(RelYear) & LiveDate_Qualifier=="Exact",
                          RelYear])
  nyrs_live <- (maxyr_live)-(minyr_live)
  ...
  if(nyrs_live>=40){
     ...
     # Set breaks to every 3 years if between 15 and 9 years of data
     brk <- 3
  }else if(nyrs_live<12 & nyrs_live>=9){
     # Set breaks to every 2 years if between 9 and 6 years of data
     brk <- 2
  } else if(nyrs_live<9){
     # Set breaks to every year if less than 6 years of data
 ...
 breaks_live <- seq(minyr_live, maxyr_live, brk)
