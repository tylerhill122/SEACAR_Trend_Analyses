...
Compiled and edited by [J.E. Panzik](jepanzik@usf.edu) for SEACAR.
...
diagnosticplots <- function(model, indicator, managedarea, sizeclass="",
  ...
  ma <- paste0(gsub('\\b(\\pL)\\pL{2,}|.','\\U\\1', managedarea, perl=TRUE),
               ifelse(str_detect(managedarea, "NERR"), "ERR", 
                      ifelse(str_detect(managedarea, "NMS"),
                             "MS",
                             ifelse(str_detect(managedarea, "AP"), "P", ""))))
  if(sizeclass != ""){
...
meplots <- function(models, data, indicator, managedarea, sizeclass="",
  ...
  if(ind=="Den"){ 
    ...
    ggsave(paste0("output/Density/Figures/Oyster_Dens_GLMM_", ma, "_", type,
  ...
  if(ind=="Pct"){
    ...
    ggsave(paste0("output/Percent_Live/Figures/Oyster_PrcLive_GLMM_", ma,
                  "_raw.png"),
           plot1,
           width=8,
           height=4,
...
meplotssh <- function(models1, data1, sizeclass1="", models2, data2,
  ...
  maxyr_hist <- max(data1[!is.na(LiveDate) & LiveDate_Qualifier=="Estimate",
                          LiveDate],
                    data2[!is.na(LiveDate) & LiveDate_Qualifier=="Estimate",
                          LiveDate])
  minyr_hist <- min(data1[!is.na(LiveDate) & LiveDate_Qualifier=="Estimate",
                          LiveDate],
                    data2[!is.na(LiveDate) & LiveDate_Qualifier=="Estimate",
                          LiveDate])
  nyrs_hist <- (maxyr_hist)-(minyr_hist)+1
  ...
  ## Live plot settings
  maxyr_live <- max(data1[!is.na(LiveDate) & LiveDate_Qualifier=="Exact",
                          LiveDate],
                    data2[!is.na(LiveDate) & LiveDate_Qualifier=="Exact",
                          LiveDate])
  minyr_live <- min(data1[!is.na(LiveDate) & LiveDate_Qualifier=="Exact",
                          LiveDate],
                    data2[!is.na(LiveDate) & LiveDate_Qualifier=="Exact",
                          LiveDate])
  nyrs_live <- (maxyr_live)-(minyr_live)+1
  ...
  if(nyrs_live>=40){
     # Set breaks to every 10 years if more than 30 years of data
     brk <- 10
  }else if(nyrs_live<40 & nyrs_live>=20){
     # Set breaks to every 5 years if between 30 and 15 years of data
     brk <- 5
  }else if(nyrs_live<20 & nyrs_live>=12){
     # Set breaks to every 3 years if between 15 and 9 years of data
     brk <- 3
  }else if(nyrs_live<12 & nyrs_live>=8){
     # Set breaks to every 2 years if between 9 and 6 years of data
     brk <- 2
  } else if(nyrs_live<8 & nyrs_live>=3){
     # Set breaks to every year if less than 6 years of data
     brk <- 1
  }else if(nyrs_live<3){
     brk <- 1
     maxyr_live <- maxyr_live + 1
     minyr_live <- minyr_live - 1
  }
  yrlist_live <- seq(minyr_live,maxyr_live,brk)
...
ab_sho75_glmm_hist <- update(ab_sho75_glmm_hist, 
                             newdata = subset(ab_sho75,
                                              ab_sho75$LiveDate_Qualifier=="Estimate"),
                             family=gaussian, cores=4,
                             control= list(adapt_delta=0.99, max_treedepth=15),
                             iter=3000, warmup=1000, chains=4, thin=3, seed=1115,
...
an_sh25to75_glmm <- update(an_sh25to75_glmm, 
                           newdata = subset(an_sh25to75,
                                            an_sh25to75$LiveDate_Qualifier!="Estimate"),
                           family=gaussian, cores=4,
                           control=list(adapt_delta=0.99, max_treedepth=15),
                           iter=3000, warmup=1000, chains=4, thin=3, seed=5699,
...
an_sho75_glmm <- update(an_sho75_glmm, 
                        newdata = subset(an_sho75,
                                         an_sho75$LiveDate_Qualifier!=
                                            "Estimate"),
                        control= list(adapt_delta=0.99, max_treedepth=15),
                        iter=3000, warmup=1000, chains=4, thin=3, seed=3639,
                        backend="cmdstanr", threads=threading(2),
...
an_sho75_glmm_hist <- update(an_sho75_glmm_hist, 
                             newdata = subset(an_sho75,
                                              an_sho75$LiveDate_Qualifier=="Estimate"),
                             family=gaussian, cores=4,
                             control=list(adapt_delta=0.99,max_treedepth=15),
...
# eb_sh2o755t_glmm <- update(eb_sh25to75_glmm,
#                            newdata = subset(eb_sh25to75,
#                                             eb_sh25to75$LiveDate_Qualifier=="Exact"),
#                            family=gaussian, cores=4,
#                            control= list(adapt_delta=0.99, max_treedepth=15),
#                            iter=3000, warmup=1000, chains=4, thin=3, seed=6881,
...
eb_sh25to75_glmm_hist <- update(eb_sh25to75_glmm_hist, 
                           newdata = subset(eb_sh25to75,
                                         eb_sh25to75$LiveDate_Qualifier=="Estimate"),
                           family=gaussian, cores=4,
                           control=list(adapt_delta=0.99, max_treedepth=20),
...
eb_sho75_glmm <- update(eb_sho75_glmm, 
                        newdata = subset(eb_sho75,
                                         eb_sho75$LiveDate_Qualifier=="Exact"),
                        family=gaussian, cores=4,
                        control=list(adapt_delta=0.99, max_treedepth=15), iter=3000,
                        warmup=1000, chains=4, thin=3, seed=3138,
                        backend="cmdstanr", threads=threading(2),
...
eb_sho75_glmm_hist <- update(eb_sho75_glmm_hist, 
                             newdata = subset(eb_sho75,
                                              eb_sho75$LiveDate_Qualifier=="Estimate"),
                             family=gaussian, cores=4,
                             control=list(adapt_delta=0.99, max_treedepth=20),
...
grm_sh25to75_glmm <- update(grm_sh25to75_glmm, 
                            newdata = subset(grm_sh25to75,
                                             grm_sh25to75$LiveDate_Qualifier=="Exact"),
                            family=gaussian, cores=4,
                            control= list(adapt_delta=0.8, max_treedepth=10),
                            iter=3000, warmup=1000, chains=4, inits=30, thin=3,
...
grm_sh25to75_glmm_hist <- update(grm_sh25to75_glmm_hist, 
                                 newdata = subset(grm_sh25to75,
                                                  grm_sh25to75$LiveDate_Qualifier=="Estimate"),
                                 family=gaussian,
                                 prior=c(set_prior("normal(6.25, 7)",
...
grm_sho75_glmm <- update(grm_sho75_glmm, 
                         newdata = subset(grm_sho75,
                                          grm_sho75$LiveDate_Qualifier=="Exact"),
                         family=gaussian, cores=4,
                         control= list(adapt_delta=0.8, max_treedepth=10),
                         iter=3000, warmup=1000, chains=4, inits=30, thin=3,
...
grm_sho75_glmm_hist <- update(grm_sho75_glmm_hist, 
                              newdata = subset(grm_sho75,
                                               grm_sho75$LiveDate_Qualifier=="Estimate"),
                              family=gaussian,
                              prior=c(set_prior("normal(7.36, 6)",
                                                class="meanme"),
                                      set_prior("normal(15.54, 4)",
                                                class="sdme"),
                                      set_prior("cauchy(0,2)",
...
gtmn_sh25to75_glmm <- update(gtmn_sh25to75_glmm, 
                             newdata = subset(gtmn_sh25to75,
                                              gtmn_sh25to75$LiveDate_Qualifier != "Estimate"),
                             family=gaussian, cores=4,
                             control=list(adapt_delta=0.8, max_treedepth=10),
                             iter=3000, warmup=1000, chains=4, inits=30, thin=3,
...
gtmn_sho75_glmm <- update(gtmn_sho75_glmm, 
                          newdata = subset(gtmn_sho75,
                                           gtmn_sho75$LiveDate_Qualifier != "Estimate"),
                          family=gaussian,
                          prior=c(set_prior("normal(171,10)",
                                            class="b",
                                            coef="RelYear"),
                                  set_prior("cauchy(0,2)")),
                          cores=4, control=list(adapt_delta=0.99, max_treedepth=10),
...
gtmn_sho75_glmm_hist <- update(gtmn_sho75_glmm_hist, 
                               newdata = subset(gtmn_sho75,
                                                gtmn_sho75$LiveDate_Qualifier=="Estimate"),
                               family=gaussian,
                               prior=c(set_prior("normal(146,25)",
...
irvbfp_sho75_glmm_hist <- update(irvbfp_sho75_glmm_hist, 
                                 newdata = subset(irvbfp_sho75,
                                                  irvbfp_sho75$LiveDate_Qualifier=="Estimate" &
                                                     !is.na(irvbfp_sho75$RelYear)),
                                 family=gaussian, cores=4,
                                 control= list(adapt_delta=0.999, max_treedepth=15),
...
lb_sho75_glmm_hist <- update(lb_sho75_glmm_hist, 
                             newdata = lb_sho75,
                             family=gaussian, cores=4,
                             control= list(adapt_delta=0.999, max_treedepth=20),
                             iter=5000, warmup=1000, chains=4, inits=75, thin=3,
...
# Calculate estimated Density_m2 values for ProgramID==5074.
# This line can be deleted after Claude recalculates in the combined table.
# I couldn't include it at the beginning of the script because I need to use
# the counts columns rather than the QuadSize_m2 column which is filled for the
# whole combined table.
...
saveRDS(irvb_n, paste0('data/GLMMs/AllDates/Data/irvb_n_',
                       Sys.Date(), '.rds'))
...
irvb_den_glmm <- brm(formula=Density_m2 ~
                   ...
                   warmup=1000, chains=4, inits=0, thin=3, seed=5512,
                   backend="cmdstanr", threads=threading(2),
                   file="data/GLMMs/AllDates/irvb_den_glmm9.rds")
...
saveRDS(jbji_n, paste0('data/GLMMs/AllDates/Data/jbji_n_',
                       Sys.Date(), '.rds'))
...
jbji_den_glmm <- brm(formula=Density_m2 ~
                   ...
                   warmup=1000, chains=4, inits=0, thin=3, seed=5512,
                   backend="cmdstanr", threads=threading(2),
                   file="data/GLMMs/AllDates/jbji_den_glmm9.rds")
...
abap_p <- subset(oysterraw_pct,
                oysterraw_pct$MA_plotlab==
                      "Apalachicola Bay Aquatic Preserve_Natural")
saveRDS(abap_p, paste0('data/GLMMs/AllDates/Data/abap_p_', Sys.Date(), '.rds'))
...
saveRDS(abap_p_binom,
        paste0('data/GLMMs/AllDates/Data/abap_p_binom_',
                            Sys.Date(), '.rds'))

abap_pct_glmm <- brm(formula=LiveObs ~ RelYear+(1 | UniversalReefID),
                    data=abap_p_binom, family=bernoulli, cores=4,
                    control= list(adapt_delta=0.99,
                                  max_treedepth=15),
                    iter=3000, warmup=1000, chains=4, inits=0,
                    thin=3,
                    seed=4331, backend="cmdstanr",
                    threads=threading(2),
                    file="data/GLMMs/AllDates/abap_pct_glmm3.rds")
...
anerr_p <- subset(oysterraw_pct,
                oysterraw_pct$MA_plotlab==
                      "Apalachicola National Estuarine Research Reserve_Natural")
saveRDS(anerr_p, paste0('data/GLMMs/AllDates/Data/anerr_p_', Sys.Date(), '.rds'))
...
saveRDS(anerr_p_binom,
        paste0('data/GLMMs/AllDates/Data/anerr_p_binom_',
                            Sys.Date(), '.rds'))

anerr_pct_glmm <- brm(formula=LiveObs ~ RelYear+(1 | UniversalReefID),
                    data=anerr_p_binom, family=bernoulli, cores=4,
                    control= list(adapt_delta=0.99,
                                  max_treedepth=15),
                    iter=3000, warmup=1000, chains=4, inits=0,
                    thin=3,
                    seed=4331, backend="cmdstanr",
                    threads=threading(2),
                    file="data/GLMMs/AllDates/anerr_pct_glmm3.rds")
...
gtm_p <- subset(oysterraw_pct,
                oysterraw_pct$MA_plotlab==
                      "Guana Tolomato Matanzas National Estuarine Research Reserve_Natural")
saveRDS(gtm_p, paste0('data/GLMMs/AllDates/Data/gtm_p_', Sys.Date(), '.rds'))
...
saveRDS(gtm_p_binom,
        paste0('data/GLMMs/AllDates/Data/gtm_p_binom_',
                            Sys.Date(), '.rds'))

gtm_pct_glmm <- brm(formula=LiveObs ~ RelYear+(1 | UniversalReefID),
                    data=gtm_p_binom, family=bernoulli, cores=4,
                    control= list(adapt_delta=0.99,
                                  max_treedepth=15),
                    iter=3000, warmup=1000, chains=4, inits=0,
                    thin=3,
                    seed=4331, backend="cmdstanr",
                    threads=threading(2),
                    file="data/GLMMs/AllDates/gtm_pct_glmm3.rds")
...
irvb_p <- subset(oysterraw_pct,
                oysterraw_pct$MA_plotlab==
                      "Indian River-Vero Beach to Ft. Pierce Aquatic Preserve_Natural")
saveRDS(irvb_p, paste0('data/GLMMs/AllDates/Data/irvb_p_', Sys.Date(), '.rds'))
...
saveRDS(irvb_p_binom, paste0('data/GLMMs/AllDates/Data/irvb_p_binom_',
                            Sys.Date(), '.rds'))

irvb_pct_glmm <- brm(formula=LiveObs ~ RelYear+(1 | UniversalReefID),
                    data=irvb_p_binom, family=bernoulli, cores=4,
                    control= list(adapt_delta=0.99, max_treedepth=15),
                    iter=3000, warmup=1000, chains=4, inits=0, thin=3,
                    seed=4331, backend="cmdstanr", threads=threading(2),
                    file="data/GLMMs/AllDates/irvb_pct_glmm3.rds")
...
jbji_p <- subset(oysterraw_pct,
                oysterraw_pct$MA_plotlab==
                      "Jensen Beach to Jupiter Inlet Aquatic Preserve_Natural")
saveRDS(jbji_p, paste0('data/GLMMs/AllDates/Data/jbji_p_', Sys.Date(), '.rds'))
...
saveRDS(jbji_p_binom, paste0('data/GLMMs/AllDates/Data/jbji_p_binom_',
                            Sys.Date(), '.rds'))

jbji_pct_glmm <- brm(formula=LiveObs ~ RelYear+(1 | UniversalReefID),
                    data=jbji_p_binom, family=bernoulli, cores=4,
                    control= list(adapt_delta=0.99, max_treedepth=15),
                    iter=3000, warmup=1000, chains=4, inits=0, thin=3,
                    seed=4331, backend="cmdstanr", threads=threading(2),
                    file="data/GLMMs/AllDates/jbji_pct_glmm3.rds")
