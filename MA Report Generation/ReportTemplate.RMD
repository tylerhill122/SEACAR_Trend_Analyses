...
```{r station_maps function}

station_maps <- function(ma){
  ...
  df_coord <- coordinates_df %>%
    filter(ManagedAreaName == ma) %>%
    select(ProgramLocationID, OriginalLatitude, OriginalLongitude)
  
  for (i in 1:nrow(df_coord)){
    lati <- df_coord$OriginalLatitude[i]
    long <- df_coord$OriginalLongitude[i]
    sta_name <- df_coord$ProgramLocationID[i]
    map <- map %>%
      addMarkers(lng=long, lat=lati, label = sta_name,
